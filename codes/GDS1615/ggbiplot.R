                     ...
                     ...)
{
  ...
  if(!is.null(df.u$labels)) {
    ...
    if(!is.null(df.u$groups)) {
      g <- g + geom_point(aes(color = groups, palette = ), alpha = alpha, size = point.size)
    } else {
  ...
  if(var.axes) {
    ...
    # Draw directions
    arrow_style <- arrow(length = unit(1/5, 'picas'), type="closed", angle=15) 
    g <- g +
      geom_segment(data = df.v,
                   aes(x = 0, y = 0, xend = xvar, yend = yvar),
                   arrow = arrow_style, 
                   color = varname.color,
                   linewidth = 0.8)    # MR: was 1.2
  }
